...
impl<T: AsRef<[u8]>> RMTopicView<T> {
    ...
    pub fn packet(&self) -> &RMWireFrameView<T> {
        &self.packet
    }
}
...
mod test {
    ...
    use crate::duss::vbus::{CMDID_VBUS_ADD_SUB, CMDID_VBUS_PUSH_MSG, CMDSET_VBUS};
    ...
    #[test]
    fn test_rm_init_packet_1() {
        let buf = [
            0x55u8, 0x4C, 0x04, 0xFF, 0x09, 0x03, 0x0F, 0x00, 0x40, 0x48, 0x03, 0x09, 0x01, 0x03,
            0x00, 0x07, 0xa7, 0x02, 0x29, 0x88, 0x03, 0x00, 0x02, 0x00, 0x66, 0x3e, 0x3e, 0x4c,
            0x03, 0x00, 0x02, 0x00, 0xfb, 0xdc, 0xf5, 0xd7, 0x03, 0x00, 0x02, 0x00, 0x09, 0xa3,
            0x26, 0xe2, 0x03, 0x00, 0x02, 0x00, 0xf4, 0x1d, 0x1c, 0xdc, 0x03, 0x00, 0x02, 0x00,
            0x42, 0xee, 0x13, 0x1d, 0x03, 0x00, 0x02, 0x00, 0xb3, 0xf7, 0xe6, 0x47, 0x03, 0x00,
            0x02, 0x00, 0x32, 0x00, 0xFF, 0xFF,
        ];

        let frame = RMWireFrameView::new(&buf);
        let topic_view = RMAddSubView::new(frame);

        assert_eq!(topic_view.packet.cmd_set(), CMDSET_VBUS);
        assert_eq!(topic_view.packet.cmd_id(), CMDID_VBUS_ADD_SUB);

        assert_eq!(topic_view.num_topics(), 7);

        std::println!("topics: {:0x?}", topic_view.topics());
    }
}
